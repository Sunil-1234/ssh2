  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <div className="flex-shrink-0">
            <a href="#home" onClick={(e) => handleNavClick(e, '#home')} className={`text-2xl font-bold transition-colors duration-300 ${isScrolled || isMenuOpen ? 'text-gray-800' : 'text-white'}`}>
              SSH <span className="text-primary">SoftTech</span>
            </a>
          </div>
          <div className="hidden md:block">
          ...
          <div className="md:hidden flex items-center">
            <button
              onClick={() => setIsMenuOpen(!isMenuOpen)}
              className={`inline-flex items-center justify-center p-2 rounded-md hover:text-primary focus:outline-none transition-colors duration-300 ${isScrolled || isMenuOpen ? 'text-gray-700' : 'text-white'}`}
            >
              <svg className="h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                {isMenuOpen ? (
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
